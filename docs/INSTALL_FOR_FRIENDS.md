...
Open Get Started and choose `Import existing Minecraft server`. If the quick scan misses it, click `Deep Search This PC`; this searches more folders and available drives, so it can take a minute. If MineHost Helper still cannot find the server, click `Browse for Server Folder` and select the main server folder, a folder just above it, or a nearby folder inside it. You can also copy the full path to the Minecraft server `.jar` from File Explorer and paste it into the manual path box. This works when modded server packs put the jar one or more folders deeper than `server.properties`. Do not select only the world folder.
